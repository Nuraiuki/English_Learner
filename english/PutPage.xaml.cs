...
namespace english
{
    public partial class PutPage : ContentPage
    {
        ...
        public PutPage()
        {
            ...
            // Получаем список фразовых глаголов, начинающихся на "put", из базы данных
            List<PhrasalVerb> putPhrasalVerbs = DatabaseManager.GetPhrasalVerbsStartingWithFromDatabase("put");

            // Привязываем список к CarouselView
            putCarousel.ItemsSource = putPhrasalVerbs;
        }

        private async void LearnButton_Clicked(object sender, EventArgs e)
        {
            // Получаем выбранный фразовый глагол из карусели
            PhrasalVerb selectedVerb = putCarousel.CurrentItem as PhrasalVerb;
        ...
        {
            // Здесь должна быть ваша реализация получения ID текущего пользователя
